...
public class EnemyCombat : MonoBehaviour
{
    ...
    {
        if(!enemyMovement.OnGround()) return;
    ...
    {
        Vector3 hitboxPosition = transform.position + new Vector3(basicAttack_HitboxDistance.x * enemyMovement.GetFaceSide(), basicAttack_HitboxDistance.y, 0);
        Collider[] hitPlayers = Physics.OverlapBox(hitboxPosition, basicAttack_HitboxSize * 0.5f, transform.rotation, playerLayer);
        foreach(Collider player in hitPlayers)
        {
            if (basicAttack_AlreadyHit.Add(player))
            {
                var (success, parry) = player.GetComponent<Entity>().Hit(basicAttack_Damage);
                if (success)
                {
                    rigidbody.velocity = new Vector3(0, 0, 0);
                    if (animator.GetCurrentAnimatorStateInfo(0).fullPathHash == BasicAttack4_StateHash)
                    {
                        player.GetComponent<Rigidbody>().AddForce(new Vector3(10 * enemyMovement.GetFaceSide(), 10, 0), ForceMode.Impulse);
                    }
                }
                else if (parry)
                {
    ...
    {
        if (enemy == null || enemy.enemyMovement == null) return;

        float face = enemy.enemyMovement.GetFaceSide();
